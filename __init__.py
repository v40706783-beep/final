...
class User(db.Model, UserMixin):
    ...
    bio = db.Column(db.Text)  # Описание профиля
...
class Notification(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    type = db.Column(db.String(50))  # comment_moderation, snippet_moderation, etc.
    title = db.Column(db.String(200))
    message = db.Column(db.Text)
    related_id = db.Column(db.Integer)  # ID связанного объекта (comment_id, snippet_id)
...
def report_comment(id):
    ...
    if existing:
        ...
        # Если накопилось 15 жалоб, отправляем на модерацию и скрываем
        if comment.reports_count >= 15 and comment.moderation_status == 'approved':
            ...
            notification = Notification(
                ...
                message=f'Ваш комментарий к сниппету "{comment.snippet.title}" отправлен на модерацию из-за множественных жалоб.',
...
def edit_profile():
    if request.method == 'POST':
        ...
        if len(bio) > 500:
            flash('Описание не должно превышать 500 символов.', 'danger')
        ...
        if 'avatar' in request.files:
            ...
            if file.filename:
                ...
                if file.mimetype not in ['image/jpeg', 'image/png', 'image/gif']:
                    flash('Поддерживаются только JPG, PNG и GIF.', 'danger')
                    return render_template('edit_profile.html')
